// Auto-generated from MTA Stations.csv
...
/**
 * Look up a station name from a GTFS-RT stop_id.
 * Stop IDs in the realtime feed include a direction suffix (N/S),
 * e.g. "R01N". We strip it to find the parent station.
 */
export function getStopName(stopId: string): string {
  if (STOPS[stopId]) return STOPS[stopId];

  const parent = stopId.replace(/[NS]$/, "");
  if (STOPS[parent]) return STOPS[parent];
