-- ─────────────────────────────────────────────────────────────
-- Settlement: detect arrivals, compute outcomes, pay winners,
-- and update historical stop_stats.
-- ─────────────────────────────────────────────────────────────

-- ─────────────────────────────────────────────────────────────
-- settle_markets: the core settlement loop
--
-- For each closed market whose predicted arrival is in the past:
--   1. Set actual_arrival = latest_predicted_arrival
--   2. Compute outcome: on_time if ≤5 min late, else late
--   3. Pay winners (2× stake) and zero-out losers
--   4. Credit winner profiles
--   5. Update stop_stats running averages
--
-- Uses a temp table to track which markets were settled in
-- this invocation, preventing double-crediting on re-runs.
-- ─────────────────────────────────────────────────────────────
...
begin
  ...
  -- Set payouts: winners get 2×, losers get 0
  update public.wagers w
  set payout = case
    when w.prediction::text = js.outcome::text then w.amount * 2
    else 0
  end
  from _just_settled js
...
-- ─────────────────────────────────────────────────────────────
-- pg_cron: run settlement every minute
-- ─────────────────────────────────────────────────────────────
select cron.schedule(
  'settle-markets',
  '* * * * *',
  $$select public.settle_markets()$$
);
